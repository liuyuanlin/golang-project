...
func (l *LockServices) ForceUnLock(req *proto.ForceUnLock, rep *proto.ForceUnLockResult) (err error) {
	l.l.RLock()
	server, exist := l.m[req.Service]
	l.l.RUnlock()
	if !exist {
		rep.Result = false
		return nil
	...
	ret, err := server.ForceUnLock(req.Name)
	rep.Result = ret
	return err
}
