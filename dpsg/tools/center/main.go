...
func main() {
	...
	common.DebugInit(centercfg.GcTime, centercfg.DebugHost)

	centerServer := center.NewCenterServer(centercfg)

	tsock, err := net.Listen("tcp", centercfg.CenterHost)
	if err != nil {
	...
	listenner, err := net.Listen("tcp", centercfg.CenterForGm)
	if err != nil {
		logger.Fatal("net.Listen: %s", err.Error())
	}
