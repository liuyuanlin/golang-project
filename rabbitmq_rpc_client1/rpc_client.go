...
type DBClient struct {
	...
	AmqpCallbackQueue amqp.Queue
	AmqpDeliveryMsg   <-chan amqp.Delivery
}
...
func (self *DBClient) Init(uri string, exchangeName string, queueName string) error {
	...
	log.Println("AmqpCallbackQueue  create")
	self.AmqpCallbackQueue, err = self.AmqpChannel.QueueDeclare("rcp_queue_callback", false, false, true, false, nil)
	if err != nil {
		log.Println("amqp Callback Queue Declare error:%v", err)
		return err
	}

	self.AmqpDeliveryMsg, err = self.AmqpChannel.Consume(
		self.AmqpCallbackQueue.Name, // queue
		"",    // consumer
		true,  // auto-ack
		true,  // exclusive
		false, // no-local
	...
	log.Printf("Consume temQuene name:%s", self.AmqpCallbackQueue.Name)
	return nil
}
...
func (self *DBClient) DBWrite(request *example.DBSetRequest) (*example.DBSetResponse, error) {
	...
	err = self.AmqpChannel.Publish(
		...
		amqp.Publishing{
			...
			MessageId:       "Set",
			Type:            "Request",
			ReplyTo:         self.AmqpCallbackQueue.Name,
			Body:            data,
		})
	if err != nil {
